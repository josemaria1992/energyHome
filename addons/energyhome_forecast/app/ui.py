...
def render_dashboard(
    ...
    <div class="card accuracy-card" id="accuracyCard">
        <h2 class="section-title">📊 Forecast Accuracy</h2>
        ...
        <div id="metricsContent" style="display: none;">
            <div class="metrics-grid">
                <div class="metric-item">
                    <div class="metric-label">Yesterday Day-Ahead Accuracy</div>
                    <div class="metric-value" id="accuracyValue">—</div>
                </div>
                <div class="metric-item">
                    <div class="metric-label">Date Evaluated</div>
                    <div class="metric-value" id="dateEvaluated">—</div>
                </div>
                <div class="metric-item">
                    <div class="metric-label">Bins Compared</div>
                    <div class="metric-value" id="binsCompared">—</div>
                </div>
            </div>
            <div class="metrics-help">
                <strong>About day-ahead forecast accuracy:</strong> This shows how well yesterday's frozen forecast
                matched the actual load. The forecast was generated before the day started and never changed during
                the day. 100% means perfect prediction. This metric is what matters for real-world planning.
            </div>
        </div>
    </div>
    """
    ...
    // Load accuracy metrics (daily forecast accuracy)
    async function loadMetrics() {
        try {
            const data = await apiGet('metrics/daily');
            renderMetrics(data);
        } catch (e) {
    ...
    function renderMetrics(data) {
        ...
        // Accuracy percentage
        const accuracy = data.accuracy_pct;
        ...
        // Date evaluated
        const dateEvaluated = data.date_evaluated || 'N/A';
        document.getElementById('dateEvaluated').textContent = dateEvaluated;

        // Bins compared
        const binsCompared = data.n_bins_compared || 0;
        document.getElementById('binsCompared').textContent = binsCompared;
    }
