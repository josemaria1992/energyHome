...
def build_day_forecast(
    ...
    """
    Build a frozen day-ahead forecast for a specific calendar date.

    Args:
        df: Historical binned data (must have ts_local column)
        target_date: The date to forecast (e.g., date(2026, 1, 14))
        timezone: The local timezone
        ilc_curve: ILC correction curves by signal
        learning_mode: "off", "weekday_profile", or "ilc_yesterday"

    Returns:
        (timestamps[96], values_by_signal) where values_by_signal is {signal: {bin: value}}
    """
    # Generate exactly 96 bins for the target date (00:00, 00:15, ..., 23:45)
    start_of_day = datetime.combine(target_date, datetime.min.time(), tzinfo=timezone)
    timestamps = [start_of_day + timedelta(minutes=15 * i) for i in range(96)]

    # Filter historical data to exclude the target date (if any partial data exists)
    if not df.empty:
        df_filtered = df[df["ts_local"].dt.date < target_date].copy()
    else:
        df_filtered = df.copy()

    outputs: Dict[str, Dict[int, float]] = {}

    for signal in ["total_w", "l1_w", "l2_w", "l3_w", "inverter_w", "grid_l1_w", "grid_l2_w", "grid_l3_w"]:
        values_dict = {}

        if learning_mode == "weekday_profile":
            # Use weekday-specific baseline
            target_dow = target_date.weekday()
            baseline = compute_baseline_weekday(df_filtered, signal, target_dow)
            baseline = smooth_baseline(baseline)

            for i, ts in enumerate(timestamps):
                bin_index = _bin_index(pd.Timestamp(ts))
                base_value = baseline.get(bin_index, 0.0)
                values_dict[i] = float(max(0.0, base_value))

        elif learning_mode == "off":
            # Use global baseline without ILC correction
            baseline = compute_baseline(df_filtered, signal)
            baseline = smooth_baseline(baseline)
            ...
            # Use global baseline + ILC correction
            baseline = compute_baseline(df_filtered, signal)
            baseline = smooth_baseline(baseline)
            curve = ilc_curve.get(signal, {})
            ...
            for i, ts in enumerate(timestamps):
                ...
                base_value = baseline.get(bin_index, 0.0)
                correction = curve.get(bin_index, 0.0)
                values_dict[i] = float(max(0.0, base_value + correction))

        outputs[signal] = values_dict
